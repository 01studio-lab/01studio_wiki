...
目标跟踪用于追踪图像中感兴趣的物体，常见的应用有自动跟随拍照。

## 实验目的

采集图像中感兴趣物体特征，然后追踪该物体。
...
```mermaid
graph TD
    导入AI视觉框架相关模块  --> 加载相关模型 --> 初始化和配置相关模块 --> 获取摄像头图像 --> 采集物体特征 --> 拍照 --> 跟踪物体 --> 画图指示 --> 拍照;
```
...
在CanMV IDE中运行上述代码，代码开始运行后出现一个绿色矩形框，将要跟踪物体放在矩形框内。

原图：

![tracker](./img/tracker/tracker0.png)

这里演示追踪葡萄，将葡萄放进绿色矩形框。

![tracker](./img/tracker/tracker1.jpg)

等待矩形框变成红色后说明学习成功并开始追踪，这时候移动摄像头，始终能找到葡萄。

![tracker](./img/tracker/tracker2.jpg)
