...
上一节我们学习过手掌关键点指的是5个手指的关节位置检测，本节进一步通过对关键点检测判断当前手势。

## 实验目的

检测摄像头拍摄到的画面中的手掌关键点实现手势分类并画图指示。
...
```mermaid
graph TD
    导入AI视觉框架相关模块  --> 加载相关模型 --> 初始化和配置相关模块 --> 获取摄像头图像 --> AI视觉推理查找图像中手掌关键点数据 --> 判读手势 --> 画图描绘 --> 获取摄像头图像;
```
...
代码中`det_boxes`变量为手掌结果， `gesture_res`为识别手势判断结果。
...
运行代码，将摄像头正对下图手掌，可以看到判断出不同的手势结果。

原图：

![hand_keypoint_class](./img/hand_keypoint_class/hand_keypoint_class1.png)

识别结果：

![hand_keypoint_class](./img/hand_keypoint_class/hand_keypoint_class2.jpg)
