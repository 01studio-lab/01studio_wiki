...
```mermaid
graph TD
    导入AI视觉框架相关模块  --> 加载相关模型 --> 初始化和配置相关模块 --> 获取摄像头图像 --> AI视觉推理查找图像中手掌 --> 画图描绘 --> 获取摄像头图像;
```
...
代码中`det_boxes`变量为人脸检测结果， `res`为识别结果。
...
![hand_detection](./img/hand_detection/hand_detection1.jpg)

识别结果：

![hand_detection](./img/hand_detection/hand_detection2.jpg)
