...
在前面学习了按键输入设备后，我们这一节先来学习输出设备LCD显示屏，其实之前的LED灯也算是输出设备，因为它们确切地告诉了我们硬件的状态。只是相对于只有亮灭的LED而言，LCD显示屏可以显示更多的信息，体验更好。

我们把它放在了前面来学习，是因为学会了LCD显示屏的使用，那么在后面的实验中可玩性就更强了。

LCD液晶显示屏是非常常见的一个外接显示设备，我们看到的手持设备、小型电器，很多都用到LCD，部分配合触摸屏应用，能实现非常多的功能。
这里主要针对4.3寸RGB/MCU屏和7寸RGB屏的讲解，学习完后你会发现，得益于MicroPython强大的移植性，他们的使用方法几乎完全一样。

## 实验平台
麦哲伦MicroPython开发套件、4.3寸RGB显示屏（电容触摸）。

![lcd](./img/4_3_rgb/4_3_rgb1.png)
...
我们先来看看实验所使用的4.3寸RGB屏 (电容触摸) 的参数介绍：[**点击购买>>**](https://item.taobao.com/item.htm?id=648500515760)

![lcd](./img/4_3_rgb/4_3_rgb2.png)
...
4.3寸RGB屏通过40P排线连接到麦哲伦H743核心板（只支持这种方式），这样可以让显示屏方便地延长出来使用。

![lcd](./img/4_3_rgb/4_3_rgb3.jpg)

也可以将排线折叠，用螺母将RGB屏固定在开发板。

![lcd](./img/4_3_rgb/4_3_rgb4.jpg)

![lcd](./img/4_3_rgb/4_3_rgb5.png)

![lcd](./img/4_3_rgb/4_3_rgb6.jpg)

实验用的RGB屏是4.3寸，驱动是的LG4572B，使用24位RGB888并口跟STM32通信，按以往嵌入式C语言开发，我们需要对LG4572B进行编程实现驱动，然后再建立各种描点、划线、以及显示图片函数。
...
我们管这些函数的集合叫驱动，驱动可以是预先在固件里面，也可以通过.py文件存放在开发板文件系统。也就是说工程师已经将复杂的底层代码封装好，我们顶层直接使用python开发即可，人生苦短。我们来看看麦哲伦开发板4.3寸RGB LCD的构造函数和使用方法。

## LCD43R对象

### 构造函数
```python
tftlcd.LCD43R(portrait=1)
```
构建4.3寸RGB屏对象。
...
```python
LCD43R.drawLine(x0, y0, x1, y1, color)
```
画线段。
- `x0`: 起点横坐标；
- `y0`: 起点纵坐标；
...
更多用法请阅读官方文档：<br></br>
https://docs.01studio.cc/library/tftlcd/tftlcd.LCD43R.html
<br></br>
...
```mermaid
graph TD
    导入tftlcd模块 --> 构建4.3寸RGB屏对象 --> 执行画点,线,图形,字符,图像功能
```
...
![lcd](./img/4_3_rgb/4_3_rgb6.png)

在Thonny IDE运行程序，可以看到LCD依次显示相关内容。

- 点、线、字符等显示

![lcd](./img/4_3_rgb/4_3_rgb7.jpg)
...
![lcd](./img/4_3_rgb/4_3_rgb8.jpg)

![lcd](./img/4_3_rgb/4_3_rgb9.jpg)
