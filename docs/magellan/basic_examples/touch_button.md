...
## 前言
上两节我们分别学习了LCD显示和触摸屏操作，这一节我们就来整合一下，做一个好玩的东西，那就是生成触摸按钮。由于麦哲伦开发板上只有3个功能按键，而使用LCD和触摸屏则可以创建非常多的按钮来执行我们的任务。而且操作更直观。

## 实验平台
麦哲伦MicroPython开发套件  + 4.3寸RGB显示屏（电容触摸）/ 7寸RGB显示屏（电容触摸）/ 4.3寸MUC屏（电容触摸） 其中一款。
...
#LCD初始化
d = LCD7R(portrait=4) #竖屏
d.fill(WHITE) #填充白色

#触摸屏初始化
t = GT911(portrait=4)#跟显示屏方向一致
...
根据自己屏的型号选择上方对应代码运行，可以看到LCD显示屏上生成了2个按钮，分别点击每个按钮，可以看到对应的LED3和LED4状态发生翻转。
