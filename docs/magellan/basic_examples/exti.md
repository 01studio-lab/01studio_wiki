...
本节我们使用A0按键作为外部中断检测按键，LED蓝灯作为指示灯。

![exti1](./img/exti/exti0.png)

本节利用中断方式来检查A0按键状态，按键被按下（产生外部中断）后使LED蓝灯(D11引脚)的状态反转。外部中断也是通过machine模块的Pin子模块来配置，我们先来看看其配构造函数和使用方法：
...
LED=Pin('D11',Pin.OUT) #构建LED对象,开始熄灭
KEY=Pin('A0',Pin.IN,Pin.PULL_UP) #构建KEY对象
state=0  #LED引脚状态
...
在Thonny IDE中运行代码，每次按下按键啊A0时候，可以看到LED蓝灯的亮灭状态翻转。

![exti](./img/exti/exti4.jpg)
