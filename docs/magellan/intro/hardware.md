...
麦哲伦H743核心板的主控为STM32H743IIT6, 该芯片内部具备2MB flash和1M SRAM，480MHZ主频、176个引脚。

![intro2](./img/product/intro2.png)

### 内存SDRAM

STM32H743系列MCU的内部RAM只有1M Bytes，因此并不能适用很多对内存要求较高的应用，麦哲伦板载了32M字节的SDRAM，用于提升主控的内存以应对更复杂的程序代码。

![intro2](./img/product/intro3.png)

### FLASH

STM32H743芯片带了2M字节的Flash，但空间非常有限，因此我们增加了16字节的Flash，相当于拥有1个16M的U盘来存放代码和图片音视频等文件。

![intro2](./img/product/intro4.png)

### BO0T键

核心板集成BOOT启动键，按下启动可以轻松进入DFU烧录固件模式。

![intro2](./img/product/intro5.png)

### LED和按键

核心板集成4路LED、1个功能按键和复位键。

![intro2](./img/product/intro6.png)

### RGB显示屏接口

核心板带RGB888显示屏接口（40P-0.5MM FPC座），可以外接01Studio 4.3寸/7寸RGB显示屏。
...
![intro2](./img/product/intro7_1.png)

### 按键

1个复位按键和3个功能按键。其中复位键和USR键跟核心板相通。

![intro2](./img/product/intro7_2.png)
...
开发板带有音频芯片，支持麦克风和线性输入，音频输出以及1个8欧/1W带音腔的喇叭。
...
提供1路RS485通讯接口。位于开发板左上方。

![intro2](./img/product/intro18.png)

### CAN总线

提供1路CAN通讯接口。位于开发板左上方，提供CAN电平转换电路，可以直接外接CAN总线通讯。需要将跳线帽打到中间和下方以连接CAN转换芯片和主控的IO。
...
麦哲伦板载DCMI摄像头接口，可以使用01Studio摄像头模块实现拍摄功能，支持排针和FPC排线连接。默认连接01Studio OV2640摄像头。FPC座使用后翻盖上接，以适应不同排线顺序的摄像头。

![intro2](./img/product/intro23.png)

### MCU显示屏接口

麦哲伦开发板板载LCD接口，可支持16位的MCU显示屏（带触控）。默认使用01Studio 4.3寸MCU LCD（带电容触摸）。
