...
pyWiFi ESP32-S3 和 pyBase开发底板，需要使用USB转TTL工具。

![timer](./img/uart/kit.png)
...
pyWiFi-ESP32-S3开发板一共有3个串口，编号0-2，如下表：其中uart0为调试串口不能使用，实验可以使用uart1或uart2。
...
- `tx` ：ESP32-S3发送引脚，可自定义IO，例：tx = 12。

- `rx` ：ESP32-S3接收引脚，可自定义IO，例：rx = 11。
...
注意要使用3.3V电平的USB转串口TTL工具，本实验我们让ESP32-S3的rx=12 , tx = 11 接线示意图如下（交叉接线）：
...
我们按照上述方式将USB转TTL的TX接到开发板的RX（9），USB转TTL的RX接到开发板的TX（8）。GND接一起，3.3V可以选择接或不接。
