...
## 实验平台
pyWiFi-ESP32-S3开发套件、3.2寸显示屏（电阻触摸）。

![lcd](./img/touch/kit.png)
...
:::tip 提示
3.2寸显示屏配套的是电阻屏，需要使用指甲或笔尖等硬物进行触摸。跟手机电容屏不一样。
:::

- 进入触摸校准，从左上角开始依次点击十字中心点
...
![touch](./img/touch/touch4.png)

重启开发板，可以看到文件系统多了一个`touch_cail`文件，在运行电阻屏初始化时候会检测这个文件，如果存在则不进行校准，若想重新校准的用户可以把这个文件删除即可！**也可以直接上传我们提供的touch_cail文件跳过校准。**
