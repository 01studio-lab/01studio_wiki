...
## 实验平台
pyWiFi-ESP32-S3开发套件和红外测温传感器模块，传感器连接到I2C/UART扩展接口。
![bmp280_1](./img/mlx90614/kit.png) 
...
本例程用到的MLX90614传感器模块，有DCC（短距离10cm）和DCI（远距离100cm）两个型号，代码通用。[**点击购买>>**](https://item.taobao.com/item.htm?id=661077456204)
...
```mermaid
graph TD
    导入相关模块-->构建MLX90614对象-->获取环境温度值/物体温度值并在OLED显示-->获取环境温度值/物体温度值并在OLED显示;
```
