...
## 实验平台
pyWiFi-ESP32-S3开发套件和继电器模块。
![relay](./img/relay/kit.jpg)
...
务必使用3.3V电平控制的继电器，因为ESP32-S3的GPIO耐压值为3.3V，使用5V控制的继电器可能会反向烧坏开发板。

:::

![relay](./img/relay/relay0.png)

左侧低压控制接口主要有供电引脚和信号控制引脚（供电电压一般为3.3V，具体以厂家参数为准）。右侧蓝色为高压部分，可连接220V电器。
...
01Studio的继电器模块的控制原理非常简单，跟LED控制方式一样，只是使用低电平‘0’表示继电器开，高电平‘1’表示继电器关。我们可以参考基础实验—按键外部中断实验例程来使用继电器，请参考  [**外部中断**](../basic_examples/exti.md) 章节内容，这里不再重复！

继电器连接到pyBase的’Y11’引脚，相当于连接到ESP32-S3的引脚10，代码编写流程如下：
