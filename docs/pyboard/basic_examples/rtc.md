...
# 定义星期和时间（时分秒）显示字符列表
rtc_week = ['Mon', 'Tues', 'Wed', 'Thur', 'Fri', 'Sat', 'Sun']
rtc_time = ['', '', '']
...
while True:
    ...
    # 显示日期，字符串可以直接用“+”来连接
    oled.text(str(datetime[0]) + '-' + str(datetime[1]) + '-' + str(datetime[2]) + ' ' + rtc_week[(datetime[3])], 0, 40)
    ...
    # 显示时间
    oled.text(rtc_time[0] + str(datetime[4]) + ':' + rtc_time[1] + str(datetime[5]) + ':' + rtc_time[2] + str(datetime[6]), 0, 55)
...
在Thonny IDE运行代码，可以看到oled屏显示当前RTC时间信息。
