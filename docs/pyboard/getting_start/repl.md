...
pyBoard的MicroPython固件集成了交互解释器REPL 【读取(Read)-运算(Eval)-输出(Print)-循环(Loop) 】，开发者可以直接通过串口终端来调试开发板。

## 连接开发板

将开发板连接到电脑，从我的**电脑—属性—设备管理器**中找到当前开发板的串口号，这里是COM10。
...
选择“MicroPython（通用）”和当前开发板对应的串口号，点击确认。
