...
## 实验平台
pyBoard开发套件和人体感应传感器模块。

![human_induction1](./img/human_induction/human1.jpg) 
...
| 输出信号  | 数字信号：<br></br> 检测到人体：高电平3.3V,持续3-5秒; <br></br>未检测到人体：低电平0V。|
| 感应角度  | 100° |
| 感应距离  | 8米 |
| 模块尺寸  | 4.5*2.5cm |
...
由此可见，可以使用外部中断结合上升沿的出发方式来编程实现相关功能。传感器的输出引脚连接Sensor1接口，即连接到pybaord的“Y11”引脚。编程方法可以是当“Y11”引脚产生中断时候，说明传感器检测到人体红外线，此时可以在OLED上显示相关信息。
...
```mermaid
graph TD
    导入相关模块--> 初始化相关模块,定义OLED闪烁函数 --> 初始化中断,回调函数指向OLED闪烁函数-->产生中断时候执行回调函数;
```
...
Human=Pin('Y11',Pin.IN,Pin.PULL_UP) #构建人体红外对象
