...
可以看到连接到DS18B20传感器是哥伦布主控的PG6引脚。也就是说针对该引脚编写程序来驱动DS18B20。那么我们需要自己来编写驱动么？如果你有兴趣的可以自己尝试一下。这部分我们01Studio已经收集整理和编写好了，当然也包括DS18B20。单总线模块文件是：onewire.py，DS18B20模块的文件是DS18X20.py，将示例程序文件夹里面以上这两个文件拷贝到开发板件系统中。
