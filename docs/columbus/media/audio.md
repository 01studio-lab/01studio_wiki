...
## 前言
哥伦布开发板上集成了音频解码芯片，可以播放音乐，本节学习一下编程实现音频文件播放。

## 实验平台
哥伦布开发套件。
![img](./img/audio/audio1.jpg)
...
哥伦布开发板上带有WM8978音频解码芯片，具备多路音频输出、麦克风和线性音频输入功能。
...
哥伦布有三个按键，我们可以分别设置用来控制音乐播放、音量加和音量减。编程思路如下：
...
本实验需要将示例代码的MP3音频文件拷贝到哥伦布开发板，大文件推荐直接U盘拷贝而非thonny上传，速度会快很多。

![img](./img/audio/audio4.png)

运行后按USR键，音乐开始播放，通过A0按键控制音量加，E3按键控制音量减。

![img](./img/audio/audio5.png)

## 实验拓展 - 触摸按键版

音乐播放除了播放和音量调整，还有暂停播放、停止等功能，但哥伦布上只有3个功能按键，因此我们可以通过LCD触摸按钮来控制，让实验操控变得更丰富。
