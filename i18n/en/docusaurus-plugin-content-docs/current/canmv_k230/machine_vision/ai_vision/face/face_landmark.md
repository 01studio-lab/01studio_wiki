...
```mermaid
graph TD
    导入AI视觉框架相关模块  --> 加载相关模型 --> 初始化和配置相关模块 --> 获取摄像头图像 --> AI视觉推理查找图像中人脸并找出关键部位 --> 画图指示 --> 获取摄像头图像;
```
...
![face_detection](./img/face_landmark/face_landmark1.jpg)

识别结果：

![face_detection](./img/face_landmark/face_landmark2.jpg)
