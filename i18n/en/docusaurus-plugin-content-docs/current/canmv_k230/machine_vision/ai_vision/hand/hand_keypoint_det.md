...
```mermaid
graph TD
    导入AI视觉框架相关模块  --> 加载相关模型 --> 初始化和配置相关模块 --> 获取摄像头图像 --> AI视觉推理查找图像中手掌关键点数据 --> 画图描绘 --> 获取摄像头图像;
```
...
代码中`det_boxes`变量为手掌结果， `hand_res`为识别手掌关键点结果。
...
![hand_detection](./img/hand_keypoint_det/hand_keypoint_det1.jpg)

识别结果：

![hand_detection](./img/hand_keypoint_det/hand_keypoint_det2.jpg)
